...
RSpec.describe Jekyll::MultiSitemap::SitemapGenerator do
  ...
  it 'concatenates two include names into one file' do
    ...
    expect(locations(built(site, 'sitemap.xml'))).to include(
      'https://example.com/', 'https://example.com/common_pages/legal.html'
    )
  end
  ...
  it 'lists a page only when sitemap.exclude is false, by default' do
    ...
    urls = locations(built(site, 'sitemap.xml'))

    expect(urls).to include('https://example.com/', 'https://example.com/about.html')
  ...
  it 'lists everything except sitemap.exclude true in opt-out mode' do
    ...
    urls = locations(built(build_site(config), 'sitemap.xml'))
  ...
  it 'writes no element for an unset lastmod, changefreq or priority' do
    entry = built(build_site(one_file), 'sitemap.xml')[%r{<url>\s*<loc>https://example.com/bare.html</loc>.*?</url>}m]
  ...
  it 'writes every element a page does set' do
    xml = built(build_site(one_file), 'sitemap.xml')
  ...
  it 'reads a lastmod of "current" as the site build time' do
    site = build_site(one_file)
    entry = built(site, 'sitemap.xml')[%r{<url>\s*<loc>https://example.com/</loc>.*?</url>}m]

    expect(entry).to include("<lastmod>#{site.time.xmlschema}</lastmod>")
  end

  it 'drops index.html from a location and makes it absolute' do
    expect(locations(built(build_site(one_file), 'sitemap.xml'))).to include('https://example.com/')
  end
  ...
  it 'matches a where value against a data path' do
    ...
    expect(locations(built(site, 'sitemap.xml'))).to eq(['https://example.com/versions/v2-guide.html'])
  end
  ...
  it 'matches a where value literally when it has no $' do
    ...
    expect(locations(built(site, 'sitemap.xml'))).to eq(['https://example.com/versions/v1-guide.html'])
  end
  ...
  it 'writes no index unless index: is set' do
    site = build_site(
      'sitemaps' => { 'path' => '/sitemap', 'files' => [{ 'name' => 'pages', 'include' => ['pages'] }] }
    )
