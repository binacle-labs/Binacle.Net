...
| `SmokeSet.cs`, `SampleSet.cs`, `CoresSet.cs` | The benchmark picks - the smoke four, the 30-problem Bischoff sample, and the 30 Bischoff problems the racing bench runs on every core count. Each answers `Names` and `GetByName(name)` |
