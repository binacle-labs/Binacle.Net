...
```json
{
  ...
  "Result": {
    ...
    "BFD": "PartiallyPacked PartiallyPacked"
  },
...
- `Result` — the **expected** outcome the tests assert against, keyed by algorithm, each key holding a
  `{PackingStatus} {FittingStatus}` pair. The keys come from the `Algorithm` enum, so a new algorithm is named
  on the next regen.
