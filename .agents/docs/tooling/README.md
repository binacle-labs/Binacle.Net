...
| `bench.just` | **The `bench` module.** One recipe per benchmark project and tier under `<slice>/bench/`, each recipe's comment its cost, and one private `<name>-run` recipe per project that holds its one line: `dotnet run -c Release --project <project> -- --job <job> --filter '*.<Tier>_*'`. Every recipe starts with `lib-` or `vipaq-`. The tier is the first word of the class name (`Smoke_`, `Sample_`, `Full_`). Smoke takes nothing; sample runs the default job and takes `quick` where it runs long; full is `[confirm]`, runs `short` and takes `precise`. `[arg(..., pattern=...)]` checks the word before anything runs; a recipe that takes one sets the job in a short bash body, then calls its `-run` recipe. Each project's `Main` is `BenchmarkProgram.Run` from `shared/test/Binacle.Benchmarking`, which exits 1 when nothing ran or a case failed - BenchmarkDotNet itself exits 0. The BDN config is C# there too, so no config file sits here. Per case, the default job costs 13-20 s whatever the method does and `short` about 5 s (read from the BDN 0.15.8 source, 2026-09-21); `short` keeps Mean and Ratio but widens Error and RatioSD, so a ratio under 1.1 needs the default job. Today: `lib-algorithms-smoke`, `-sample`, `-full`; `lib-racing-smoke`, `-sample`; `lib-threshold-smoke`, `-sample`, `-full`; `lib-result-selection`; `vipaq-smoke`, `-sample` |
