...
| `measure.just` | **Not a script** — the `measure` module for the root `justfile`. `just measure lib` and `just measure vipaq` run the slice's measure project (`lib/measure/Binacle.Lib.PackingEfficiency`, `vipaq/measure/Binacle.ViPaq.EncodedSize`), which writes its reports into the slice's tracked `results/`; `just measure all` both; `just measure check` runs all, then fails if either `results/` changed - the same golden-file check as `just regen check` |
| `bench.just` + `bench.run.sh` | **The `bench` module plus its one script.** One recipe per benchmark project and tier under `<slice>/bench/`, each recipe's comment its cost, all of them doors onto `bench.run.sh <project> <job> [categories...] [-- flags]`. The tier (`smoke`, `sample`, `full`) and the narrowing words (`ffd`, `bfd`, `wfd`, `packing`, `fitting`) are `[BenchmarkCategory]` values on the classes and rows; the script passes them as `--allCategories` (every one must match) and everything from the first `-` word on to BenchmarkDotNet as is: `dotnet run -c Release --project <project> -- --job <job> --allCategories <categories> <flags>`. Three owners: the recipe knows project, job, tier and cost; the script only calls `dotnet run`; the C# says which case is in which tier. The BDN config is C# in `shared/test/Binacle.Benchmarking`, so no config file sits here. Today: `lib-algorithms` (= `-sample`), `-smoke`, `-full` (`job="default"`, prints the count and estimates first), `lib-racing` (= `-full`, default job), `lib-racing-smoke`, `lib-threshold` (= `-sample`), `-smoke`, `-full` (`job="default"`) and `lib-result-selection`; `vipaq` arrives when its project lands |
...
The launch profiles live in `serve.just`; the benchmark word table lives in `bench.run.sh`.
