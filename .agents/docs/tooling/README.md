...
agent indexes, the build, the image stacks and the smoke suite, plus the benchmarks, local Docker
...
| Script | What it does |
|---|---|
| `serve.just` | **Not a script** — the `serve` module for the root `justfile`, everything run **from source**. `just serve api [profile]` runs the API via `dotnet run -lp <profile>` (`Normal`/`WithServiceModuleOnly`/`WithUiModuleOnly`/`WithAllModules`, aliases `N/S/U/All`, default `Normal`); `just serve docs`, `just serve demo`, `just serve www` and `just serve admin` run jekyll + webpack watch together from `sites/<site>`; `just serve services-up [-d]` / `just serve services-down` bring up what the API talks to |
...
| `measure.just` | **Not a script** — the `measure` module for the root `justfile`. `just measure lib` and `just measure vipaq` run the slice's measure project (`lib/measure/Binacle.Lib.PackingEfficiency`, `vipaq/measure/Binacle.ViPaq.EncodedSize`), which writes its reports into the slice's tracked `results/measurements/`; `just measure all` both |
| `bench.just` | **The `bench` module.** One recipe per benchmark project and tier under `<slice>/bench/`, each recipe's comment its cost, and one private `<name>-run` recipe per project that holds its one line: `dotnet run -c Release --project <project> -- --job <job> --filter '*.<Tier>_*'`; `lib-result-selection` and `lib-scaling`, one class set and one tier each, run `--filter '*'`. Every recipe starts with `lib-` or `vipaq-`. The tier is the first word of the class name (`Smoke_`, `Sample_`, `Full_`; racing's one tier is `Cores_`, run like a full tier). Smoke takes nothing; sample runs the default job and takes `quick` where it runs long; full is `[confirm]`, runs `short` and takes `precise`. `[arg(..., pattern=...)]` checks the word before anything runs; a recipe that takes one sets the job in a short bash body, then calls its `-run` recipe. Each project's `Main` is `BenchmarkProgram.Run` from `shared/test/Binacle.Benchmarking`, which exits 1 when nothing ran or a case failed - BenchmarkDotNet itself exits 0. The BDN config is C# there too, so no config file sits here. Per case, the default job costs 13-20 s whatever the method does and `short` about 5 s (read from the BDN 0.15.8 source, 2026-09-21); `short` keeps Mean and Ratio but widens Error and RatioSD, so a ratio under 1.1 needs the default job. Today: `lib-algorithms-smoke`, `-sample`, `-full`; `lib-racing-cores`; `lib-threshold-smoke`, `-sample`, `-full`; `lib-result-selection`; `lib-scaling`; `vipaq-smoke`, `-sample` |
...
The launch profiles live in `serve.just`.
