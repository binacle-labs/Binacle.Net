...
| `tests.just` | **Not a script** — the `test` module for the root `justfile`. One recipe per suite, run with `just test <name>`, plus three group recipes — `image`, `sites` and `all`. The tests are `[private]`, so completion offers the three groups; a bare `just test` prints every test name. The two test lists in it are the only copy of the set of tests, and `all` is their sum with nothing run twice. A group is for a laptop; CI names every test as its own step. See `$commands` for the list |
...
| `check.just` | **Not a script** — the `check` module for the root `justfile`. `just check links` checks the internal links in all three built sites with `lychee --offline`, `just check links <site>` one of them, `just check links-external <site>` every link including other people's servers, `just check workflows` runs actionlint over `.github/workflows`, and `just check actions` greps the `.github/actions` manifests for the `vars`/`secrets` expression actionlint cannot see. Both list the files they were handed and end with a count, so a clean run reads differently from one that never started. Reads `artifacts/<site>`, so the site has to be built first — it stops with a pointer to `just build <site>` rather than checking nothing. `tooling/check.lychee.toml` holds the URLs it must never check and is named with `--config`, because lychee only finds a config in the working directory by itself |
...
| `ci.just` + `ci/*.sh` | **The `ci` module plus one script per operation.** The shell a workflow runs, kept out of the YAML: a `.just` body and a `run:` block are both invisible to shellcheck and neither can be run on its own. `ci.just` is a door - two lines per recipe - and `tooling/ci/<name>.sh` is the code. Sixteen operations today: `changed-paths`, `gate`, `deploy-message`, `push-tag`, `deploy-summary`, `sonar-summary`, `check-release-ref`, `check-version`, `check-release-tag`, `changelog-section`, `github-release`, `release-summary`, `pull-image`, `smoke-summary`, `codeql-summary`, `dockerhub-version`. Every one takes its inputs as arguments and reads no `github.*` context, so all of them run on a laptop; the ones that print `key=value` are teed into `$GITHUB_OUTPUT` and the ones that write a summary fall back to `/dev/stdout`. See `$ci-cd` |
