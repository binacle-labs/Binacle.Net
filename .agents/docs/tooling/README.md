...
| `bench.just` + `bench.run.sh` | **The `bench` module plus its one script.** One recipe per benchmark project and tier under `<slice>/bench/`, each recipe's comment its cost, all of them doors onto `bench.run.sh <project> <job> [words...]`. The script turns the words `ffd`, `bfd`, `wfd`, `packing`, `fitting` into one `--filter` glob, always `*<Op>*<Alg>*` whatever order the words come in (two globs would be OR) and passes any other word to BenchmarkDotNet as is, then `dotnet run -c Release --project <project> -- --job <job> --filter <glob>`. The BDN config is C# in `shared/test/Binacle.Benchmarking`, so no config file sits here. Today: `lib-result-selection` at the `short` job; the rest arrive as the split lands |
