...
| `agents.just` | **Not a script** — the `agents` module for the root `justfile`. `just agents all` regenerates the `_index.md` manifest for `.agents/rules`, `.agents/docs`, `.agents/design`, `.agents/plans` and `.agents/memory` (grouped by area); `just agents generate-index <name>` does one |
